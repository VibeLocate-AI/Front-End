<template>
  <div class="app-container" :class="{ 'auth-mode': !['Landing', 'Home', 'HomeAlias', 'Rent', 'Map', 'Profile', 'ProfileTab'].includes(route.name) }">
    <template v-if="!['Landing', 'Home', 'HomeAlias', 'Rent', 'Map', 'Profile', 'ProfileTab'].includes(route.name)">
...
watch(() => route.name, async (name) => {
  if (!['Landing', 'Home', 'Rent', 'Map', 'Profile', 'ProfileTab'].includes(name)) {
    await nextTick()
